...
public class WBDBSafetyMstInsertDTO extends BaseDTO {
    ...
    private List<WBDBSafetyDtlDTO> rsumeDtlList;

    @Schema(title = "신청 진행 상세 리스트")
    private List<WBDBSafetyPbsnDtlDTO> pbsnDtlList;

    @Schema(title = "상생신청지원금액상세")
    private List<WBDBSafetySpprtDTO> spprtList;

    @Schema(title = "신청 진행 파일 리스트")
    private List<WBDBSafetyFileDtlDTO> fileDtlList;
