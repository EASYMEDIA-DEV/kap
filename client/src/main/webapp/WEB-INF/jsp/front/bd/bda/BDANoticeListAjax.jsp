                               ...
                               data-details-key="${mainPostList.ntfySeq}">
                                ...
                                <div class="txt-box">
                                    ...
                                    <div class="sub-txt f-body2"><p
                                            class="date">${ kl:convertDate(mainPostList.regDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy.MM.dd', '') }</p>
                                        <p class="view">조회수 <span>${mainPostList.readCnt}</span></p></div>
                                </div>
                            </a>
                        </c:forEach>
                    </c:when>
                    <c:when test="${mainPostData.mainPostList.size() > 0 && mainPostData.mainPostList.size() < 3}">
                        <c:forEach var="mainPostList" items="${mainPostData.mainPostList}" varStatus="status1" begin="0"
                                   end="${mainPostData.mainPostList.size()}">
                            <a class="list-item noticeListView mainPost open" href="javascript:" title="링크 이동"
                               data-details-key="${mainPostList.ntfySeq}">
                                <div class="sub-info-wrap">
                                    <span class="info f-body1">중요</span>
