    ...
    <c:when test="${ not empty rtnData.list}">
        <c:forEach var="list" items="${rtnData.list}" varStatus="status">
            <tr data-total-count="${rtnData.totalCount}">
                ...
                <td class="text-center" data-type-cd="${list.typeCd}>${ list.typeCd}</td>
                <td class="text-center">
                    <a href="./write?seq=${list.seq}" class="listView" data-details-key="${list.seq}">
                            ${ list.titl}
                    </a>
                </td>
                ...
                <td class="text-center" data-use-yn="${list.seq}">${ kl:decode(list.useYn, 'Y', '노출', '미노출') }</td>
